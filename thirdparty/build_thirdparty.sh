...
if [ -n "$F_ALL" -o -n "$F_THRIFT" ]; then
  if [ "$(uname)" == "Darwin" ]; then
    ...
    CXXFLAGS="$CXXFLAGS -fPIC" ./configure --without-tests --without-qt4 --without-c_glib --without-csharp --without-java --without-erlang --without-nodejs --without-lua --without-python --without-perl --without-php --without-php_extension --without-ruby --without-haskell --without-go --without-d --with-cpp --prefix=$PREFIX
